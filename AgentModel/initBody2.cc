...
double AgentModel::initBody(XMLNode& part1Node, std::shared_ptr<Joint> joint)
{
  ...
  if (part1Node.getName() == "bodypart")
  {
    ...
    string path = "/conf/agentmodel/joint[@part1='" + part1Node.getProp("name") + "']";
    ...
    for (XMLNodeSet::iterator iter = set.begin(); iter != set.end(); ++iter)
    {
      ...
      std::string type = jointNode.getProp("type");

      if (type == "universal")
      {
        j->type = Types::UNIVERSAL_JOINT;
        j->ujindex = atoi(jointNode.getProp("ujindex").c_str());
        if (j->ujindex == 0)
          cochlea.setTranslation(j->perceptor, j->name);
      }
      else if (type == "hinge" || type == "")
      {
        j->type = Types::HINGE_JOINT;
        j->ujindex = 0;
        cochlea.setTranslation(j->perceptor, j->name);
      }
      else if (type == "torque")
      {
        j->type = Types::TORQUE_JOINT;
        j->ujindex = 0;
        cochlea.setTranslation(j->perceptor, j->name);
      }
      ...
      j->bounds.first = Math::degToRad(atof(jointNode.getProp("min").c_str()));
      j->bounds.second = Math::degToRad(atof(jointNode.getProp("max").c_str()));
      ...
      path = "/conf/agentmodel/*[@name='" + jointNode.getProp("part2") + "']";
      _debugLevel4(path);
      XMLNodeSet set2 = conf.selectXPath(path);
      XMLNode part2Node = set2.front();
      weight += initBody(part2Node, j);
    }
  }
  else
  {
    ...
    std::string type = part1Node.getProp("type");
    ...
    string path = "/conf/agentmodel/*[@name='" + part1Node.getProp("part2") + "']";
    _debugLevel4(path);
    XMLNodeSet set = conf.selectXPath(path);
