...
namespace bats
{
  /** Primitive Behavior
   *
   * This class depicts a special kind of behaviors: primitive
   * behaviors. These behaviors form the leafs of an agent's behavior
   * tree and are wrappers of the primitive actions that can be
   * performed by an agent and sent to the server (e.g. move a joint
   * and beam). New primitive behaviors should inherit from this class
   * and implement the following methods:
   *
   * <dl>
   *   <dt>bool run()</dt>
   *
   *   <dd>This method is called when the primitive behavior is
   ...
   *   the capability of performing its action in a certain
   *   state.</dd>
   * </dl>
   */
  ...
  protected:
    /**
     * PrimitiveBehaviors have no slots, so we don't have to
     * generate goals for them.  We do have to overload
    ...
    /**
     * Primitive behaviors need to implement this method to perform
     * actions;
     */
    virtual bool run() = 0;
    
  public:
    PrimitiveBehavior(std::string const &name, std::string const &id)
      : Behavior(name, id)
    {
    }
    
    PrimitiveBehavior(std::string const &name, std::string const &id,
		      std::string const& behaviorTree)
      : Behavior(name, id, behaviorTree)
    {
    }
    ...
    /** Reset the behavior to his initial state. Removes this behavior
     * from the list of action-command behaviors
     */
    virtual void reset()
    {
